-- ROI Section Database Schema with Enhanced AI-Based Metrics
-- This schema provides comprehensive tracking for ROI data with detailed analytics
...
CREATE TABLE IF NOT EXISTS roi_cards (
  ...
  icon_url TEXT,
  metric_type VARCHAR(100) DEFAULT 'percentage', -- percentage, currency, number, time
  trend VARCHAR(50) DEFAULT 'up', -- up, down, stable
  trend_percentage DECIMAL(10,2),
  previous_value VARCHAR(100),
  time_period VARCHAR(100), -- e.g., "Last 6 months", "Q4 2024"
  category VARCHAR(100), -- e.g., "conversion", "revenue", "efficiency", "engagement"
  display_order INTEGER DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS roi_metrics (
  ...
  metric_unit VARCHAR(50), -- %, $, hours, conversions, etc.
  baseline_value DECIMAL(20,4),
  target_value DECIMAL(20,4),
  achievement_rate DECIMAL(5,2), -- percentage of target achieved
  confidence_score DECIMAL(5,2), -- AI confidence in the metric (0-100)
  data_source VARCHAR(255), -- Where this metric comes from
  calculation_method TEXT, -- How this metric is calculated
  last_updated TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  is_ai_generated BOOLEAN DEFAULT FALSE,
  ai_insight TEXT, -- AI-generated insight about this metric
  predicted_trend VARCHAR(50), -- predicted, increasing, decreasing, stable
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- Case Studies Table for Detailed ROI Examples
CREATE TABLE IF NOT EXISTS roi_case_studies (
  id SERIAL PRIMARY KEY,
  roi_section_id INTEGER NOT NULL REFERENCES roi_section(id) ON DELETE CASCADE,
  ...
  key_improvements JSONB, -- Array of improvements like [{"area": "conversion", "improvement": "45%"}]
  success_metrics JSONB, -- Detailed success metrics
...
-- Industry Benchmarks Table for AI-Powered Comparisons
CREATE TABLE IF NOT EXISTS roi_industry_benchmarks (
  id SERIAL PRIMARY KEY,
  ...
  year INTEGER,
  quarter VARCHAR(10),
  percentile_25 DECIMAL(20,4), -- 25th percentile
  percentile_50 DECIMAL(20,4), -- Median
  percentile_75 DECIMAL(20,4), -- 75th percentile
  percentile_90 DECIMAL(20,4), -- 90th percentile (top performers)
  sample_size INTEGER,
  geographic_region VARCHAR(100),
  company_size_segment VARCHAR(100), -- small, medium, large, enterprise
...
CREATE INDEX IF NOT EXISTS idx_roi_cards_category ON roi_cards(category);
CREATE INDEX IF NOT EXISTS idx_roi_cards_featured ON roi_cards(is_featured);
CREATE INDEX IF NOT EXISTS idx_roi_metrics_card ON roi_metrics(roi_card_id);
CREATE INDEX IF NOT EXISTS idx_roi_case_studies_section ON roi_case_studies(roi_section_id);
CREATE INDEX IF NOT EXISTS idx_roi_case_studies_published ON roi_case_studies(is_published);
CREATE INDEX IF NOT EXISTS idx_roi_benchmarks_industry ON roi_industry_benchmarks(industry_name);
CREATE INDEX IF NOT EXISTS idx_roi_benchmarks_metric ON roi_industry_benchmarks(metric_name);
...
VALUES (
  ...
  'https://res.cloudinary.com/your-cloud/video/upload/v1/roi-demo.mp4',
  'https://res.cloudinary.com/your-cloud/image/upload/v1/roi-dashboard-1.jpg',
  'https://res.cloudinary.com/your-cloud/image/upload/v1/roi-dashboard-2.jpg',
  TRUE
...
-- Insert demo AI metrics for the first card
...
-- Insert industry benchmarks for comparison
...
DROP TRIGGER IF EXISTS update_roi_section_updated_at ON roi_section;
...
DROP TRIGGER IF EXISTS update_roi_cards_updated_at ON roi_cards;
...
DROP TRIGGER IF EXISTS update_roi_case_studies_updated_at ON roi_case_studies;
